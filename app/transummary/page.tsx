...
} from "../../src/utils/firestoreHelpers"; // Import the required Firebase functions
    ...
    if (parsedSessionData && transactionId && parsedImage) {
      try {
        ...
        if (participantSnapshot.exists()) {
          ...
          parsedSessionData.id = await generateParticipantId(
            parsedSessionData.college
          );
        }
        ...
        for (const [eventName, teamNames] of Object.entries(
          parsedSessionData.eventMap
        )) {
      ...
      alert(
        "Transaction ID, registration data and Transaction image are required."
      );
      setIsOverlayVisible(false);
    }
  };
    ...
    <div className="bg-[#0d0d0d] text-white font-sans mb-10">
      ...
      <div>
        <p className="text-2xl mt-12 text-center font-inter text-gray-200">
          Finally, verify your payment!
        </p>
      </div>
      <div className="flex flex-col items-center justify-center mt-24">
        <Image src={success} alt="Payment Success" />
        <h1 className="text-2xl font-semibold font-inter text-white text-center mx-2 my-8">
          Payment Success!
        </h1>
      </div>
      {/* <div
